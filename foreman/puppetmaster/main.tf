...
resource "openstack_compute_instance_v2" "foreman_puppetmaster" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sleep 20",
      "sudo apt update -y",
      "sudo hostnamectl set-hostname www.${replace(self.name, "_", "")}.openstacklocal",                                                                            #${replace(var.name, "_", "")}
      "sudo bash -c 'echo \"${openstack_compute_instance_v2.foreman_puppetmaster.access_ip_v4} www.${replace(self.name, "_", "")}.openstacklocal\" >> /etc/hosts'", #${replace(var.name, "_", "")}
